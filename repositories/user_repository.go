...
type UserRepository interface {
	...
	FindByPublicID(public_id string) (*models.User, error)
...
func (r *userRepository) FindByPublicID(public_id string) (*models.User, error) {
	var user models.User
	err := config.DB.Where("public_id = ?", public_id).First(&user).Error
	return &user, err
}
...
func (r *userRepository) FindAllPagination(filter, sort string, limit, offset int) ([]models.User, int64, error) {
	...
	if filter != "" {
		...
		db = db.Where("name Ilike ? OR email Ilike ?", filterPattern, filterPattern) // ILike = case insensitive
	}
